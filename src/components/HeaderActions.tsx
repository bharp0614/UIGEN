...
import { downloadFile, downloadProjectAsZip } from "@/lib/export";
import {
  Popover,
  PopoverContent,
  PopoverTrigger,
...
export function HeaderActions({ user, projectId }: HeaderActionsProps) {
  ...
  const { selectedFile, getFileContent, getAllFiles } = useFileSystem();
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <Popover open={exportOpen} onOpenChange={setExportOpen}>
        <PopoverTrigger asChild>
          <Button variant="outline" className="h-8 gap-2" title="Export code">
        ...
        <PopoverContent className="w-[220px] p-2" align="end">
          <div className="flex flex-col gap-1">
            ...
            >
              ...
              <span className="text-sm">Download current file</span>
            </Button>
            <Button
              variant="ghost"
              className="justify-start gap-2 h-9 px-2"
              onClick={handleDownloadZip}
              disabled={getAllFiles().size === 0}
            >
              <Download className="h-4 w-4" />
